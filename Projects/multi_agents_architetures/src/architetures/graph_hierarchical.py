...
from architetures.tools import python_repl, web_search
from architetures.state import AgentState
...
hierarchical_llm = ChatOpenAI(
    ...
    base_url=os.getenv("TAVILY_API_KEY")
)
...
def searcher_agent(state: AgentState):
    """搜索员代理 - 使用搜索工具获取最新信息"""
    search_agent = create_agent(hierarchical_llm, tools=[web_search])
...
def calculator_agent(state: AgentState):
    """计算员代理 - 使用代码进行计算"""
    calc_agent = create_agent(hierarchical_llm, tools=[python_repl])
...
def coder_agent(state: AgentState):
    """代码员代理 - 编写和执行代码"""
    code_agent = create_agent(hierarchical_llm, tools=[python_repl])
...
# 编译图
graph_hierarchical = hierarchical_workflow.compile()
graph_hierarchical.name = "Hierarchical-Multi-Agent"