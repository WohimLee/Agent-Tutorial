...
# 创建模型实例
network_llm = ChatOpenAI(model="Qwen 3 Max",
                        api_key=os.getenv('LLM_API_KEY'),
                        base_url=os.getenv("TAVILY_API_KEY"))

# 创建代理
network_search_agent = create_agent(
    network_llm,
    tools=[web_search]
)

network_code_agent = create_agent(
    network_llm,
    tools=[python_repl]
)
...
# 编译网络图
graph_network = network_workflow.compile()
graph_network.name = "Network-Agent"