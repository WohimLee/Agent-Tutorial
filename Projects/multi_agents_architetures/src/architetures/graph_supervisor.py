...
from architetures.tools import python_repl, web_search  # , add_sale, delete_sale, update_sale, query_sales  # SQL工具暂时注释
from architetures.state import AgentState
# TypedDict 和 Literal 导入已移除，不再需要结构化输出


# 用于普通问答对话
chat_llm = ChatOpenAI(model="Qwen 3 Max",
                   api_key=os.getenv('LLM_API_KEY'),
                   base_url=os.getenv("BASE_URL"))

# 用于数据库检索
db_llm = ChatOpenAI(model="Qwen 3 Max",
                   api_key=os.getenv('LLM_API_KEY'),
                   base_url=os.getenv("BASE_URL"))
...
# 编译图
graph_supervisor = workflow.compile()

graph_supervisor.name = "SuperVisor-Multi-Agent"
